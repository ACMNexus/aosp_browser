...
class Tab implements PictureListener {
    ...
    protected void capture() {
        ...
        if (ReflectUtils.getInstance().getContentWidth(mMainView) <= 0 || mMainView.getContentHeight() <= 0) {
            return;
        }
        Canvas c = new Canvas(mCapture);
        final int left = mMainView.getScrollX();
        final int top = mMainView.getScrollY() + ReflectUtils.getInstance().getVisibleTitleHeight(mMainView);
        int state = c.save();
